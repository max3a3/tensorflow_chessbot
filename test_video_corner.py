...
def test_findChessboardCorners(image_path):
    ...
    try:
        ...
        if corners is not None:
            ...
            # Save cropped chessboard image
            try:
                # Crop the original PIL image using the detected corners
                # corners format: [x0, y0, x1, y1] where (x0,y0) is top-left, (x1,y1) is bottom-right
                cropped_img = img_pil.crop((corners[0], corners[1], corners[2], corners[3]))
                
                # Save the cropped image
                output_path = "result.png"
                cropped_img.save(output_path)
                print(f"💾 Cropped chessboard saved as: {output_path}")
                print(f"   Cropped image size: {cropped_img.size[0]} x {cropped_img.size[1]} pixels")
                
            except Exception as crop_error:
                print(f"⚠️  Warning: Failed to save cropped image: {str(crop_error)}")
        else:
...
def main():
    ...
    if corners is not None:
        ...
        print(f"   Cropped chessboard saved as: result.png")
    else:
